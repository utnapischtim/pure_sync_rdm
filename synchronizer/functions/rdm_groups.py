...
class RdmGroups:
    ...
    #   ---         ---         ---
    def rdm_create_group(self: object, group_externalId: str, group_name: str):
        ...
        response = self.os.system(command)
    ...
    #   ---         ---         ---
    def rdm_add_user_to_group(self: object, user_id: int, group_externalId: str):
        ...
        if not response:
            ...
            rdm_create_group(self, group_externalId)
    ...
    #   ---         ---         ---
    def rdm_group_split(self: object, old_group_externalId: str, new_groups_externalIds: list):
    ...
    #   ---         ---         ---
    def rdm_group_merge(self: object, old_groups_externalId: list, new_group_externalId: str):
        """ 
        1 - Create new group
    ...
    def rdm_split_modify_record(self, old_group_externalId, report_name, new_groups_data, new_groups_externalIds):
        ...
        response = rdm_get_metadata_by_query(self, old_group_externalId)
        ...
        for item in resp_json['hits']['hits']:
            ...
            # Update record
            response = update_rdm_record(self, record_json, recid)
    ...
    def rdm_split_users_from_old_to_new_group(self, old_group_id, report_name, old_group_externalId, new_groups_externalIds):
        ...
        if not response:
            ...
            for i in response:
                ...
                # Get user email
                user_email = get_user_email(self, user_id)
                
                for new_group_externalId in new_groups_externalIds:
                    # Add user to new groups
                    group_add_user(self, user_email, new_group_externalId, user_id)

                # Remove user from old group
                response = group_remove_user(self, user_email, old_group_externalId)
    ...
    #   ---         ---         ---
    def pure_get_organisationalUnit_data(self: object, externalId: str):
    ...
    def group_remove_user(self, user_email, group_name):
        ...
        if not response:
            open(self.report_name, "a").write(f'{report} - Already removed from group\n')
            return True
        ...
        if response != 0:
            add_to_full_report(f'Warning - Creating group response: {response}')
            open(self.report_name, "a").write(f'{report} - Error: {response}\n')
            return False

        open(self.report_name, "a").write(f'{report} - Success\n')
        return True
