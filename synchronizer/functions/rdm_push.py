                ...
                if 'file' in EV:
                    if 'fileURL' in EV['file'] and 'fileName' in EV['file']:
                        ...
                        if response.status_code < 300:
                            open(str(my_prompt.dirpath) + '/reports/temporary_files/' + file_name, 'wb').write(response.content)
...
def post_to_rdm(my_prompt):
    ...
    try:
        ...
        response = my_prompt.requests.post(rdm_api_url_records, headers=headers, params=params, data=data_utf8, verify=False)

        # RESPONSE CHECK
        print('Metadata post: ', response)                

        # adds metadata http response codes into array
        if response.status_code in my_prompt.cnt_resp:       my_prompt.cnt_resp[response.status_code] += 1
        else:                                           my_prompt.cnt_resp[response.status_code] =  1
        ...
        if response.status_code >= 300:
            ...
            if response.status_code != 429:
                filename = f'{my_prompt.dirpath}/reports/temporary_files/error_jsons/{uuid}.json'
        ...
        if response.status_code < 300:
            ...
            if len(my_prompt.record_files) > 0:
                ...
                if file_resp_code not in my_prompt.cnt_resp:
                    my_prompt.cnt_resp[file_resp_code] = 0

                my_prompt.cnt_resp[file_resp_code] += 1
            ...
            open(my_prompt.dirpath + "/reports/all_rdm_records.log", "a").write(uuid_recid_line)
